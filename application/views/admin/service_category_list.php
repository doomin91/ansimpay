...
<body class="bg-1">
	...
	<!-- Make page fluid -->
		...
		<!-- page header -->
			...
			<h2><i class="fa fa-puzzle-piece" style="line-height: 48px;padding-left: 5px;"></i> <b>서비스 추가/변경</b> <span></span></h2>
			<div class="breadcrumbs">
				<ol class="breadcrumb">
					<li>서비스 관리</li>
					<li class="active">서비스 추가/변경</li>
				</ol>
			</div>
					...
					<section class="tile color transparent-black">
						...
						<div class="tile-body">
							<div style="float:right">
								<a onclick="showCreateModal()" role="button" class="btn btn-success btn-sm" data-toggle="modal">서비스 추가</a>
							</div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h3 class="modal-title" id="cateLabel">서비스 추가</h3>
				</div>
	...
	<script>
			...
			$.ajax({
				...
				success: function(data){
					...
					data.forEach(function(element, index){
						str += `
								<tr>
								<td>${element.SC_ORDER_NUMBER}</td>
								<td>${element.SC_CATEGORY_NAME}</td>
								<td>${element.SC_REG_DATE}</td>
								`
						jsonData = JSON.stringify(element)
						str += `<td>`
						if(index == 0){
							str += `<button onclick="moveUp(${element.SC_ORDER_NUMBER})" class="btn btn-xs btn-default" disabled>▲올리기</button>`
						} else {
							str += `<button onclick="moveUp(${element.SC_ORDER_NUMBER})" class="btn btn-xs btn-default">▲올리기</button>`
						}
								
						if(index == length - 1){
							str += `<button onclick="moveDown(${element.SC_ORDER_NUMBER})" class="btn btn-xs btn-default" disabled>▼내리기</button>`
						} else {
							str += `<button onclick="moveDown(${element.SC_ORDER_NUMBER})" class="btn btn-xs btn-default">▼내리기</button>`
						}
						str += `</td>`
						str += `
								<td>
									<button onclick="showModifyModal(${escape(JSON.stringify(element))})" class="btn btn-xs btn-default">수정</button>
									<button onclick="deleteCategory(${element.SC_SEQ})" class="btn btn-xs btn-danger">삭제</button>
								</td>
								</tr>
								`
					})
		...
		function showModifyModal(data){
			...
			$("input[name=cateSeq]").val(data.SC_SEQ);
			$("input[name=cateName]").val(data.SC_CATEGORY_NAME);
		...
		function moveUp(orderNumber) {
			$.ajax({
				url: "/adm/ServiceCategory/moveUp?orderNumber="+orderNumber,
				dataType: "text",
				type: "get",
				success: function(data){
					loadData()
		...
		function moveDown(orderNumber) {
			$.ajax({
				url: "/adm/ServiceCategory/moveDown?orderNumber="+orderNumber,
				dataType: "json",
				type: "get",
				success: function(data){
					loadData()
		...
		function inputCategory(){
			...
			$.ajax({
				url: 		 "/adm/ServiceCategory/inputServiceCate",
				type: 		 "post",
				data: 		 formData,
				dataType:	 "json",
				contentType: false,
		...
		function deleteCategory(scSeq){
			if(confirm("삭제하시겠습니까?")){
				$.ajax({
					url: 		 "/adm/ServiceCategory/delServiceCate",
					type: 		 "post",
					data: 		 {
						"scSeq": scSeq
					},
